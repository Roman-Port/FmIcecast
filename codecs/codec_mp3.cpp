...
fmice_codec_mp3::fmice_codec_mp3(int sampleRate, int channels, int blockSize) : fmice_codec(sampleRate, channels) {
    //Initialize encoder
    create_encoder();
    ...
    output_buffer_size = (int)(1.25 * blockSize + 7200);
...
void fmice_codec_mp3::reset_safe() {
    //Destroy stream encoder
    if (gfp != NULL) {
        lame_close(gfp);
        gfp = NULL;
...
bool fmice_codec_mp3::write_safe(float* samples, int count) {
    //Encode
